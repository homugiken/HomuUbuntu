...
dbgmsg_printf (
    ...
    ...)
{
    DBGMSG_CTL * const                  ctl = gctl->dbgmsg;
    static DBGMSG_MSG                   _msg, * const msg = &_msg;

    if (ctl == NULL) { return; }
    if (ctl->ready != true) { return; }
    if (ctl->qid < 0) { return; }
    ...
    msgsnd(ctl->qid, msg, (sizeof(time_t) + sizeof(pid_t) + DBGMSG_SRC_NAME_LEN + strlen(msg->text)), IPC_NOWAIT);
}
...
{   ENTR();
    ...
    ERR_RANGE(cfg->key_id, DBGMSG_KEY_ID_MIN, DBGMSG_KEY_ID_MAX);
